...
int main()
{
    ...
    m.metallic = 1.0;
    m.clearcoat = 0.6;
    readObj("./Resources/Models/stanford-bunny.obj", triangles, m, getTransformMatrix(vec3(0, 30, 0), vec3(0.4, -1.3, -2), vec3(0.8, 0.8, 0.8)), true);
    
    m.baseColor = vec3(0.1, 0.75, 0.2);
    m.roughness = 0.1;
    m.specular = 1.0;
    m.subsurface = 1.0;
    m.metallic = 0.1;
    ...
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -0.2, -3), vec3(2, 2, 2)), false);

    m.baseColor = vec3(1, 1, 1);
    m.metallic = 1;
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(-90, 0, 0), vec3(0.0, -1.2, -2.0), vec3(2, 2, 2)), false);

    m.baseColor = vec3(0, 1, 0);
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(0, 90, 0), vec3(1.0, -0.2, -2.0), vec3(2, 2, 2)), false);

    m.baseColor = vec3(1, 0, 0);
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(0, -90, 0), vec3(-1.0, -0.2, -2.0), vec3(2, 2, 2)), false);

    m.baseColor = vec3(1, 1, 1);
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(90, 0, 0), vec3(0, 0.8, -2.0), vec3(2, 2, 2)), false);
    
    m.baseColor = vec3(1, 1, 1);
    m.emissive = vec3(20, 20, 20);
    readObj("./Resources/Models/quad.obj", triangles, m, getTransformMatrix(vec3(90, 0, 0), vec3(0.0, 0.79, -2.0), vec3(0.5, 0.5, 0.5)), false);
    ...
    bool r = HDRLoader::load("./Resources/hdr.hdr", hdrRes);
    ...
    // 管线配置
    Shader shaderpass1("./Resources/pass1.vs", "./Resources/fshader.frag");
    ...
    Shader shaderpass2("./Resources/pass1.vs", "./Resources/pass2.frag");
    ...
    Shader shaderpass3("./Resources/pass1.vs", "./Resources/pass3.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        mat4 cameraRotate = lookAt(eye, vec3(0, -0.5, -2), vec3(0, 1, 0));  // 相机注视着原点
